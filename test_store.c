...
void test_put() {
    ...
    assert(LOGSTORE_OK == logstore_open(&s, "log"));
    for (int i=0; i<ENTRY_COUNT; ++i) 
        assert(LOGSTORE_OK == logstore_put(s, i, &i, sizeof(i), 0));
    struct stat lst;
...
void test_get() {
    ...
    for (int i=0; i<ENTRY_COUNT; ++i) {
        void *data = NULL;
        size_t sz = 0;
        assert(LOGSTORE_OK == logstore_get(s, i, &data, &sz, NULL));
...
void test_confict_detection() {
    ...
    void *data_b = NULL;
    size_t sz_a, sz_b;
    logstore_revision rev_a, rev_b;
    assert(LOGSTORE_OK == logstore_get(s, 0, &data_a, &sz_a, &rev_a));
    assert(LOGSTORE_OK == logstore_get(s, 0, &data_b, &sz_b, &rev_b));
    assert(LOGSTORE_OK == logstore_put(s, 0, data_a, sz_a, rev_a));
    // at this point the revision of entry with id 0 is 2
    assert(LOGSTORE_ECONFLICT == logstore_put(s, 0, data_b, sz_b, rev_b));
    assert(LOGSTORE_OK == logstore_close(&s));
    free(data_a);
    free(data_b);
}
...
void test_remove() {
    ...
    void *data = NULL;
    size_t sz;
    assert(LOGSTORE_OK == logstore_get(s, 0, &data, &sz, NULL));
    free(data);
    assert(LOGSTORE_OK == logstore_rm(s, 0));
    data = NULL;
    assert(LOGSTORE_ENOENT == logstore_get(s, 0, &data, NULL, NULL));   // removed!
    // way beyond any existing entry's ID:
    assert(LOGSTORE_EINVAL == logstore_rm(s, (uint64_t) -1));    
    assert(LOGSTORE_OK == logstore_close(&s));
}
